...
router.route("/geteventIdData").post(async function (req, res, next) {
	try {
		...
		eventArray.push(newInstance);
		populateHeap(eventArray,newInstance);
	} catch (error) {
		console.log("error (try-catch) : " + error);
		return res.status(500).json({
			success: false,
...
function isNewEvent(){
	// console.log("Heap Size: ", eventHeap.size());
...
async function geteventsdata(_deployHash, _timestamp, _block_hash, _eventname, _eventdata, res){
	try {
		...
		// 	});
		// }
        let _updateEvent = await event_Id_Data_Model.findOne({eventName:_eventname,deployHash:_deployHash});
		let newData = _eventdata;//req.body.eventdata;
		let deployHash = _deployHash;//req.body.deployHash;
		let timestamp = _timestamp;//req.body.timestamp;
		let block_hash = _block_hash;//req.body.block_hash;
		let eventName = _eventname;//req.body.eventname;
		console.log("... Deployhash: ", deployHash);
		console.log("... Timestamp: ", timestamp);
		console.log("... Block hash: ", block_hash);
		// console.log("Event Data: ", newData);
