...
router.route("/geteventIdData").post(async function (req, res, next) {
	try {
		if (!req.body.eventId) {
			return res.status(400).json({
				success: false,
				message: "There is no eventId specified in the req body.",
			});
		}
		...
		const newInstance = new event_Id_Data_Model({
			eventId: eventId,
		...
		populateHeap(newInstance);
    return res.status(200).json({
			success: true,
			message: "Event Received Successfully",
		});
	} catch (error) {
		console.log("error GetEvent ID : " + error);
		return res.status(500).json({
			success: false,
			err: error,
		});
...
async function populateDatabase(_eventId, _deployHash, _eventName, _timestamp, _block_hash, _eventdata){
	const newInstance = new event_Id_Data_Model({
			eventId: _eventId,
...
async function geteventsdata(_deployHash, _timestamp, _block_hash, _eventname, _eventdata){	
...
try {
    ...
    if (eventName == "pair_created") {
      ...
      if (flag == 0) {
        ...
        console.log("handleTransfer Mutation did not called.");
        return true;
        // return res.status(200).json({
        //   success: true,
      ...
      if (pairagainstuserresult == null) {
        ...
        if (
          new bigdecimal.BigDecimal(pairagainstuserresult.reserve0) >
          new bigdecimal.BigDecimal(pairagainstuserresult.reserve1)
        ) {
          if (new bigdecimal.BigDecimal(reserve0) > new bigdecimal.BigDecimal(reserve1)) {
            pairagainstuserresult.reserve0 = (
              new bigdecimal.BigDecimal(pairagainstuserresult.reserve0).add(new bigdecimal.BigDecimal(reserve0))
            ).toString();
            pairagainstuserresult.reserve1 = (
              new bigdecimal.BigDecimal(pairagainstuserresult.reserve1).add(new bigdecimal.BigDecimal(reserve1))
            ).toString();
            await pairagainstuserresult.save();
          } else {
            pairagainstuserresult.reserve0 = (
              new bigdecimal.BigDecimal(pairagainstuserresult.reserve0).add(new bigdecimal.BigDecimal(reserve1))
            ).toString();
            pairagainstuserresult.reserve1 = (
              new bigdecimal.BigDecimal(pairagainstuserresult.reserve1).add(new bigdecimal.BigDecimal(reserve0))
            ).toString();
            await pairagainstuserresult.save();
          }
        } else {
          if (new bigdecimal.BigDecimal(reserve1) > new bigdecimal.BigDecimal(reserve0)) {
            pairagainstuserresult.reserve0 = (
              new bigdecimal.BigDecimal(pairagainstuserresult.reserve0).add(new bigdecimal.BigDecimal(reserve0))
            ).toString();
            pairagainstuserresult.reserve1 = (
      ...
      if (pairagainstuserresult == null) {
        ...
        if (liquidity == "0") {
          ...
          if (
            ...
            if (new bigdecimal.BigDecimal(reserve0) > new bigdecimal.BigDecimal(reserve1)) {
              pairagainstuserresult.reserve0 = (
                new bigdecimal.BigDecimal(pairagainstuserresult.reserve0).subtract(new bigdecimal.BigDecimal(reserve0))
              ).toString();
              pairagainstuserresult.reserve1 = (
                new bigdecimal.BigDecimal(pairagainstuserresult.reserve1).subtract(new bigdecimal.BigDecimal(reserve1))
              ).toString();
              await pairagainstuserresult.save();
            } else {
              pairagainstuserresult.reserve0 = (
                new bigdecimal.BigDecimal(pairagainstuserresult.reserve0).subtract(new bigdecimal.BigDecimal(reserve1))
              ).toString();
              pairagainstuserresult.reserve1 = (
                new bigdecimal.BigDecimal(pairagainstuserresult.reserve1).subtract(new bigdecimal.BigDecimal(reserve0))
              ).toString();
              await pairagainstuserresult.save();
            }
          } else {
