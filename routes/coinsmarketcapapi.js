        ...
        for (var i = 0; i < response.data.length; i++) {
          ...
          if (response.data[i].symbol == req.body.symbol) {
            return res.status(200).json({
              ...
              worthforamountpassed:
                req.body.amount * response.data[i].quote.USD.price,
            });
          }
          ...
          if (i == response.data.length - 1) {
            console.log(
              req.body.symbol +
                " not found in the coinmarketcap Api cryptocurrencies"
            );
            return res.status(400).json({
              success: true,
              message:
      ...
      .then((response) => {
        ...
        for (var i = 0; i < response.data.length; i++) {
          ...
          if (response.data[i].symbol == req.body.symbolforconversion) {
            ...
            console.log(
              "symbolforconverion worth in USD: ",
              symbolforconvertionworthinUSD
            );
            count++;
            flag = 1;
          }
          if (response.data[i].symbol == req.body.symboltoconvertto) {
            ...
            console.log(
              "symboltoconverttoworthinUSD worth in USD: ",
              symboltoconverttoworthinUSD
            );
            count++;
            flag = 2;
          }
          if (count == 2) {
            return res.status(200).json({
              success: true,
              symbolforconversionWorthInUSD:
                req.body.symbolforconversion +
                " worth in USD = " +
                symbolforconvertionworthinUSD,
              symboltoconverttoWorthInUSD:
                req.body.symboltoconvertto +
                " worth in USD = " +
                symboltoconverttoworthinUSD,
              ratio:
                "1 " +
                req.body.symboltoconvertto +
                " = " +
                symboltoconverttoworthinUSD / symbolforconvertionworthinUSD +
                " " +
                req.body.symbolforconversion +
                " ($" +
                symboltoconverttoworthinUSD +
                ")",
              worth:
                symbolforconvertionworthinUSD / symboltoconverttoworthinUSD,
              worthforamountpassed:
                (symbolforconvertionworthinUSD / symboltoconverttoworthinUSD) *
                req.body.amounttoconvertto,
            });
          }
          if (i == response.data.length - 1) {
            if (flag == 1) {
