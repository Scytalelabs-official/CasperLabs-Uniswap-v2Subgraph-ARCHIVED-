  ...
  try {
    ...
    if (pairresult.length == 0) {
      ...
      for (var i = 0; i < pairresult.length; i++) {
        if (
          pairresult[i].token0.id == req.body.token0 &&
          pairresult[i].token1.id == req.body.token1
        ) {
      ...
      if (paircontractHash != null) {
        ...
        for (var i = 0; i < pairsresult2.length; i++) {
          if (
            pairsresult2[i].contractHash.toLowerCase() ==
            paircontractHash.toLowerCase()
          ) {
          ...
          if (i == pairsresult2.length - 1) {
            if (pairpackageHash == null) {
              return res.status(400).json({
                success: false,
                message:
                  "There is no pair against this contract Hash in the hashesofpairs collection.",
              });
            }
          }
        }
