...
const { ZERO_BD, factoryContract, ADDRESS_ZERO, ONE_BD, UNTRACKED_PAIRS } = require('./helpers');
...
async function getEthPriceInUSD() {
  ...
  if (daiPair !== null && usdcPair !== null && usdtPair !== null) {
    let totalLiquidityETH = daiPair.reserve1+(usdcPair.reserve1)+(usdtPair.reserve0);
    ...
    return daiPair.token0Price
      .times(daiWeight)
      +(usdcPair.token0Price.times(usdcWeight))
      +(usdtPair.token1Price.times(usdtWeight));
    // dai and USDC have been created
  } else if (daiPair !== null && usdcPair !== null) {
    let totalLiquidityETH = daiPair.reserve1+(usdcPair.reserve1);
    let daiWeight = daiPair.reserve1/(totalLiquidityETH);
    let usdcWeight = usdcPair.reserve1/(totalLiquidityETH);
    return daiPair.token0Price.times(daiWeight)+(usdcPair.token0Price.times(usdcWeight));
...
async function findEthPerToken(token){
  ...
  for (let i = 0; i < WHITELIST.length; ++i) {
    ...
    if (pairAddress.toHexString() != ADDRESS_ZERO) {
      let pair = await Pair.findOne({id:pairAddress.toHexString()});
...
async function getTrackedLiquidityUSD(
  ...
  // both are whitelist tokens, take average of both amounts
  if (WHITELIST.includes(token0.id) && WHITELIST.includes(token1.id)) {
    return tokenAmount0.times(price0)+(tokenAmount1.times(price1));
  }
